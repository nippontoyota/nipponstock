...
export default function AftersalesModal({ isOpen, onClose }: { isOpen: boolean; onClose: () => void }) {
  ...
  // The SSO magic link for the Aftersales Dashboard
  // Uses port 3000 by default for Next.js, but this can be changed to the production URL later
  const aftersalesUrl = "https://aftersales-dashboard-ashen.vercel.app/api/sso?token=nippon-ceo-token-2026";
      ...
      <div className="bg-zinc-950 border border-zinc-800 shadow-2xl w-full h-full max-w-[1600px] flex flex-col overflow-hidden relative animate-in fade-in zoom-in-95 duration-200">
        ...
        <div className="flex items-center justify-between px-6 py-4 bg-zinc-900 border-b border-zinc-800">
          <div className="flex items-center gap-3">
            <span className="material-symbols-outlined text-purple-500 text-2xl">build_circle</span>
            <h2 className="text-white font-headline font-bold text-lg tracking-wide uppercase">
              Aftersales Dashboard <span className="text-zinc-500 text-sm ml-2">(SECURE SESSION)</span>
            </h2>
          </div>
          <button 
            onClick={onClose}
          ...
          {loading && (
            <div className="absolute inset-0 flex flex-col items-center justify-center z-20 bg-[#120708]">
              <div className="relative">
                <div className="animate-spin rounded-full h-16 w-16 border-4 border-zinc-800 border-t-purple-500"></div>
