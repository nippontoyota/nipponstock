...
async function main() {
  for (const [old, newC] of PAIRS) {
    const oldExists = await prisma.vehicle.findUnique({ where: { chassisNumber: old } });
    const newExists = await prisma.vehicle.findUnique({ where: { chassisNumber: newC } });
    console.log(`old ${old}: ${oldExists ? '✅ EXISTS' : '❌ not found'}  |  new ${newC}: ${newExists ? '✅ EXISTS' : '❌ not found'}`);
  }
