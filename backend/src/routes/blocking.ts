...
async function revealChassis(vehicleId: string): Promise<void> {
  ...
  if (!vehicle) return;
  const entry = await prisma.chassisMap.findUnique({ where: { maskedKey: vehicle.chassisNumber } });
  if (!entry) return;
  await prisma.vehicle.update({ where: { id: vehicleId }, data: { chassisNumber: entry.realChassis } });
  console.log(`[reveal] ${vehicle.chassisNumber} → ${entry.realChassis}`);
}
